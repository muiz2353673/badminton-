...
export async function generateBracket(tournamentId: string, event: string): Promise<{
  message: string;
  count?: number;
  ...
  const res = await fetch(`${API_URL}/generate-bracket`, {
    ...
    body: JSON.stringify({ tournament_id: tournamentId, event }),
  });
  if (!res.ok) {
...
export async function getDraws(
  tournamentId: string,
  event?: string
): Promise<{ tournament_id: string; event_filter: string | null; events: string[]; matches: import("./supabase").Match[] }> {
